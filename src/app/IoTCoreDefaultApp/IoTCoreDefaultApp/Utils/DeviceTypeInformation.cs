...
{
    public enum DeviceTypes { RPI2, RPI3, MBM, DB410, PINE64, BPI_M64, GenericBoard, Unknown };
    public static class DeviceTypeInformation
    {
        ...
        static void Init()
        {
            ...
            if (_type == DeviceTypes.Unknown)
            {
                ...
                else if (deviceInfo.SystemProductName.IndexOf("Pine", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    _type = DeviceTypes.PINE64;
                }
                else if(deviceInfo.SystemProductName.IndexOf("BPI", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    _type = DeviceTypes.BPI_M64;
                }

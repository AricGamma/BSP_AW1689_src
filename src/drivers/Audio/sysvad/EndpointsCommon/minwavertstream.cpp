	...
	_In_ NTSTATUS TransferStatus)
{
	...
	PADAPTERCOMMON  pAdapterComm = NULL;
	LARGE_INTEGER Qpc = { 0 };

	//FunctionEnter();

	UNREFERENCED_PARAMETER(TransferStatus);
	...
	//DPF_ENTER("Bytes transferred %lu, Status 0x%lx.", BytesTransferred, TransferStatus);

	pStream = (CMiniportWaveRTStream *)pContext;
	pAdapterComm = pStream->m_pMiniport->GetAdapterCommObj();

	//TODO:
	Qpc = KeQueryPerformanceCounter(NULL);
	pStream->m_ullDmaTimeStamp = KSCONVERT_PERFORMANCE_TIME(pStream->m_ullPerformanceCounterFrequency.QuadPart, Qpc);

	pStream->m_ullWritePosition = (pStream->m_ullWritePosition + BytesTransferred) % pStream->m_ulDmaBufferSize;
	pStream->m_ullPlayPosition = pStream->m_ullWritePosition;
	pStream->m_ullLinearPosition += BytesTransferred;

	// TODO:
	if (0 == BytesTransferred % (pStream->m_ulDmaBufferSize / pStream->m_ulNotificationsPerBuffer))
	{
		InterlockedIncrement64(&pStream->m_llPacketCounter);
	    //DbgPrint_I("Counter increased to %lld.", pStream->m_PacketCounter);
	}

	//DPF(D_ERROR, "Current packet counter %lld.", pStream->m_llPacketCounter);

	//// Simple buffer underrun detection.
	//if (!pStream->IsCurrentWaveRTWritePositionUpdated())
	//{
	...
	//		pStream->GetCurrentWaveRTWritePosition(),
	//		1,      // WaveRT buffer is underrun
	//		0);
	//}

	if (!IsListEmpty(&pStream->m_NotificationList))
	{
		PLIST_ENTRY leCurrent = pStream->m_NotificationList.Flink;
		while (leCurrent != &pStream->m_NotificationList)
		{
...
)
{
    ...
    *CacheType_ = MmCached;
	...
	DPF(D_ERROR, "Allocated buffer MDL 0x%p, virtual address 0x%p, size %lu.", pBufferMdl, m_pDmaBuffer, RequestedSize_);
...
)
{
	...
	if (NULL != Latency_)
	{
		...
		{
			ULONG CodecDelay = 10 * 1000000; // 100-ns
			CodecDelay = m_ulDmaBufferSize * CodecDelay / (m_pWfExt->Format.nSamplesPerSec * (m_pWfExt->Format.wBitsPerSample / 8) * m_pWfExt->Format.nChannels);
			//DbgPrint_E("Codec delay [0x%lu 100-ns].", CodecDelay);
			Latency_->ChipsetDelay = 0;
			Latency_->CodecDelay = CodecDelay;
			Latency_->FifoSize = 32 * 8;
			Latency_->FifoSize = 128 * 3;
		}
	}
...
)
{
    ...
    *Flags = 0;
    ...
    {
		DPF(D_ERROR, "No new packets are available.");
    ...
    {
        // Trace a glitch
		DPF(D_ERROR, "%lu packets dropped.", droppedPackets);
    }
...
)
{
	...
	//DPF(D_ERROR, "Current write packet number %lld, expected packet number %lu.", m_llPacketCounter, expectedPacket);
    ...
    {
		DPF(D_ERROR, "Current packet number %lu, expected packet number %lu, data later.", PacketNumber, expectedPacket);
        return STATUS_DATA_LATE_ERROR;
    }
    else if (deltaFromExpectedPacket > 0)
    {
		DPF(D_ERROR, "Current packet number %lu, expected packet number %lu, data overrun.", PacketNumber, expectedPacket);
...
)
{
	...
	CODEC_FUNCTION_INTERFACE *pCodecInterface = NULL;
	I2S_FUNCTION_INTERFACE *pI2sInterface = NULL;
	I2S_TRANSMISSION_CALLBACK TransmissionCallback = { 0 };
	...
	DPF(D_ERROR, "Set to state [%lu], Operation [%s].", State_, m_bCapture ? "Capture" : "Render");

	pAdapterComm = m_pMiniport->GetAdapterCommObj();
	pCodecInterface = (m_pMiniport->GetAdapterCommObj())->GetCodecFunctionInterface();
	pI2sInterface = (m_pMiniport->GetAdapterCommObj())->GetI2sFunctionInterface();
	if ((NULL == pCodecInterface) || (NULL == pI2sInterface))
	{
		DPF(D_ERROR, "Invalid function interface table.");
		ntStatus = STATUS_INVALID_DEVICE_STATE;
		goto Exit;
	}

	ntStatus = pCodecInterface->CodecSetStreamState(pCodecInterface->InterfaceHeader.Context, m_bCapture, State_);
	if (!NT_SUCCESS(ntStatus))
	{
		DPF(D_ERROR, "Failed to set stream state to codec with 0x%lx.", ntStatus);
		goto Exit;
	}
    ...
    switch (State_)
    {
        case KSSTATE_STOP:
            // Reset DMA
			ntStatus = pI2sInterface->I2SStopTransfer(pI2sInterface->InterfaceHeader.Context, m_bCapture);
			if (!NT_SUCCESS(ntStatus))
			{
				DPF(D_ERROR, "Failed to stop I2S transmition with 0x%lx.", ntStatus);
				goto Exit;
			}
        ...
        case KSSTATE_RUN:
			...
			MAP_GUID_TO_MODE(m_SignalProcessingMode, Mode);

			DPF(D_ERROR, "Current mode %lu, samplerate %lu, channels %d, bits %d, block %d.", 
				Mode, m_pWfExt->Format.nSamplesPerSec, m_pWfExt->Format.nChannels, m_pWfExt->Format.wBitsPerSample, m_pWfExt->Format.nBlockAlign);

			ntStatus = pCodecInterface->CodecSetStreamMode(pCodecInterface->InterfaceHeader.Context, m_bCapture, (ULONG)Mode);
			if (!NT_SUCCESS(ntStatus))
			{
				DPF(D_ERROR, "Failed to set stream mode to codec with 0x%lx.", ntStatus);
				goto Exit;
			}

			ntStatus = pCodecInterface->CodecSetStreamFormat(pCodecInterface->InterfaceHeader.Context, m_bCapture, m_pWfExt);
			if (!NT_SUCCESS(ntStatus))
			{
				DPF(D_ERROR, "Failed to set stream format to codec with 0x%lx.", ntStatus);
				goto Exit;
			}

			ntStatus = pI2sInterface->I2SSetStreamFormat(pI2sInterface->InterfaceHeader.Context, m_bCapture, m_pWfExt);
			if (!NT_SUCCESS(ntStatus))
			{
				DPF(D_ERROR, "Failed to set stream mode to I2S with 0x%lx.", ntStatus);
				goto Exit;
			}
			// Start DMA
			TransmissionCallback.pCallbackContext = this;
			TransmissionCallback.pCallbackRoutine = (PI2STransmissionCallbackRoutine)NotificationRT;

			if (FALSE == m_bCapture)
			{
				ntStatus = pI2sInterface->I2SSetTransmitCallback(pI2sInterface->InterfaceHeader.Context, &TransmissionCallback);
				if (!NT_SUCCESS(ntStatus))
				{
					DPF(D_ERROR, "Failed to set I2S transmission callback with 0x%lx.", ntStatus);
					goto Exit;
				}

				DPF(D_BLAB, ("Start render."));
				ntStatus = pI2sInterface->I2STransmit(pI2sInterface->InterfaceHeader.Context, m_pMdl, m_ulDmaBufferSize, m_ulNotificationsPerBuffer);
				if (!NT_SUCCESS(ntStatus))
				{
					DPF(D_ERROR, "Failed to start I2S transmit with 0x%lx.", ntStatus);
					goto Exit;
				}
			}
			else
			{
				ntStatus = pI2sInterface->I2SSetReceiveCallback(pI2sInterface->InterfaceHeader.Context, &TransmissionCallback);
				if (!NT_SUCCESS(ntStatus))
				{
					DPF(D_ERROR, "Failed to set I2S transmission callback with 0x%lx.", ntStatus);
					goto Exit;
				...
				DPF(D_BLAB, ("Start capture."));
				ntStatus = pI2sInterface->I2SReceive(pI2sInterface->InterfaceHeader.Context, m_pMdl, m_ulDmaBufferSize, m_ulNotificationsPerBuffer);
				if (!NT_SUCCESS(ntStatus))
				{
					DPF(D_ERROR, "Failed to start I2S receive with 0x%lx.", ntStatus);
					goto Exit;
				}
			}

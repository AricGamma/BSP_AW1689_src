    ...
    )
{
	...
	DbgPrintEx(1, 0, "-------------------------------PL011EvtInterruptDpc is enter!\r\n");
    ...
    //
    if ((interruptEventsToHandle & RECEIVD_AVAILABLE) != 0) 
	{
    ...
    // trigger level.
    //
    if ((interruptEventsToHandle & THR_EMPTY) != 0) 
	{
    ...
    PL011DeviceRecordErrors(
        devExtPtr, 
        (interruptEventsToHandle & (UART_INTERUPPTS_ERRORS | UART_BI))
        );
    ...
    )
{
    ...
    ULONG regUARTRIS = PL011HwReadRegisterUlong(
        PL011HwRegAddress(DevExtPtr, UART_IIR)//UARTRIS
        );
    ...
    // Basically if RX FIFO is not empty.
    //
    if ((regUARTRIS & (RECEIVD_AVAILABLE)) != 0) //UARTRIS_RXIS | UARTRIS_RTIS
	{
        //
        // Copy new data from RX FIFO to PIO RX buffer.
        //

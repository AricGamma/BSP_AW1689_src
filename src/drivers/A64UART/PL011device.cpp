    ...
    )
{
    ...
    if ((PL011ErrorEventsMask & UART_OE) != 0) {

        newErrorTypes |= SERIAL_ERROR_OVERRUN;
    }

    if ((PL011ErrorEventsMask & UART_FE) != 0) {

        newErrorTypes |= SERIAL_ERROR_FRAMING;
    }

    if ((PL011ErrorEventsMask & UART_PE) != 0) {

        newErrorTypes |= SERIAL_ERROR_PARITY;
    }

    if ((PL011ErrorEventsMask & UART_BI) != 0) {
    ...
    )
{
    ...
    //************A64 DONOT HAVE SACH  REG  by gsy************************
    //
    ...
    )
{
    ...
    devExtPtr->UartSupportedControlsMask = 0;
    ...
    )
{
    ...
    //
    {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            PL011_LOG_ERROR(
                "SerCx2PioReceiveCreate failed, (status = %!STATUS!)", status
                );
            return status;
        }
    ...
    //
    {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            PL011_LOG_ERROR(
                "PL011TxPioReceiveInit failed, (status = %!STATUS!)", status
                );
            return status;
        }
    ...
    switch (uartFlowControlParams) 
	{
		...
		case UART_SERIAL_FLAG_FLOW_CTL_NONE:
		{
			if ((drvExtPtr->UartControlLines & UART_SERIAL_LINES_RTS) != 0) 
			{
				...
				devExtPtr->UartSupportedControlsMask |= UART_RTS;//UARTCR_RTS;
			}

			if ((drvExtPtr->UartControlLines & UART_SERIAL_LINES_DTR) != 0) 
			{

				devExtPtr->UartSupportedControlsMask |= UART_DTR;//UARTCR_DTR;
			}
			break;
		...
		case UART_SERIAL_FLAG_FLOW_CTL_HW:
		{
			if ((drvExtPtr->UartControlLines & UART_SERIAL_LINES_RTS) != 0) {
				...
				devExtPtr->UartSupportedControlsMask |= UART_FLOWCONTROL_En;//UARTCR_RTSEn;
			}

			if ((drvExtPtr->UartControlLines & UART_SERIAL_LINES_CTS) != 0) {

				devExtPtr->UartSupportedControlsMask |= UART_FLOWCONTROL_En;//UARTCR_CTSEn;
			}
			break;

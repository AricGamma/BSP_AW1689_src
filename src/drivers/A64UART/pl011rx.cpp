    ...
    )
{
    ...
    DbgPrintEx(1, 0, "+++++++++++++PL011RxPioReceiveStart!\r\n");
    PL011_DEVICE_EXTENSION* devExtPtr = PL011DeviceGetExtension(WdfDevice);
    PL011_SERCXPIORECEIVE_CONTEXT* rxPioPtr =
...
{
	DbgPrintEx(1, 0, "PL011RxPioReceiveStop!\r\n");
    PL011_DEVICE_EXTENSION* devExtPtr = PL011DeviceGetExtension(WdfDevice);
    PL011_SERCXPIORECEIVE_CONTEXT* rxPioPtr =
        PL011SerCxPioReceiveGetContext(devExtPtr->SerCx2PioReceive);
    ...
    )
{
    ...
    while (totalBytesCopied < Length) 
	{
		...
		if( Length == 1 )
		{	
			totalBytesCopied += PL011pRxPioBufferCopyTheLatestByte(
				devExtPtr,
		...
		//last_data = PL011RxPendingByteCount(rxPioPtr);

		DbgPrintEx(1, 0, "PLBufferPtrr=0x%lx,", BufferPtr);
        //
        // RX FIFO -> RX buffer
        //
    ...
    )
{
	...
	DbgPrintEx(1, 0, "-------------------------------PL011SerCx2EvtPioReceiveEnableReadyNotification is enter!\r\n");
    ...
    )
{
	...
	volatile ULONG* regUART_LSRtr = PL011HwRegAddress(DevExtPtr, UART_LSR);
 
   //see uart_dr state
    if( (PL011HwReadRegisterUlong(regUART_LSRtr) & UART_DR) !=0)
	{
	    volatile ULONG* regUARTDRPtr = PL011HwRegAddress(DevExtPtr, UART_RBR);  // status must be check first
	
    // Flags register address
   // volatile ULONG* regUARTFRPtr = PL011HwRegAddress(DevExtPtr, UARTFR);
    volatile ULONG* regUARTFRPtr = PL011HwRegAddress(DevExtPtr, UART_USR);
    ...
    while (PL011RxPendingByteCount(rxPioPtr) < PL011_RX_BUFFER_SIZE_BYTES) 
	{
        ...
        //
        if ((PL011HwReadRegisterUlong(regUARTFRPtr) & UART_RFNE) == 0) 
		{
...
_Use_decl_annotations_
ULONG
PL011pRxPioBufferCopyTheLatestByte(
	PL011_DEVICE_EXTENSION* DevExtPtr,
	UCHAR* BufferPtr,
	ULONG Length
)
{
	...
	DbgPrintEx(0, 1, "PL011pRxPioBufferCopy");
	PL011_SERCXPIORECEIVE_CONTEXT* rxPioPtr =
		PL011SerCxPioReceiveGetContext(DevExtPtr->SerCx2PioReceive);
	...
	if ((rxOut + bytesToCopy) <= PL011_RX_BUFFER_SIZE_BYTES)
	{
		...
		*/
	{
		rxOut = (rxOut + bytesToCopy-1) % PL011_RX_BUFFER_SIZE_BYTES;


	}

	*BufferPtr = rxPioPtr->RxBuffer[rxOut];
	

	rxPioPtr->RxBufferOut += bytesToCopy;


	//DbgPrintEx(1, 0, "PLBufferPtrr=0x%ld,", BufferPtr);
	//_Analysis_assume_(bytesCopied <= Length);
	//RtlCopyMemory(BufferPtr, &rxPioPtr->RxBuffer[rxOut], bytesCopied);
	...
	//rxPioPtr->RxBufferOut = rxOut;
	InterlockedAdd(&rxPioPtr->RxBufferCount, -LONG(bytesToCopy));
    ...
    )
{
    ...
    if( (PL011HwReadRegisterUlong(regUART_LSRtr) & UART_DR) !=0)
	{
		...
		volatile ULONG* regUARTDRPtr = PL011HwRegAddress(devExtPtr, UART_RBR);
		ULONG purgedBytes = 0;

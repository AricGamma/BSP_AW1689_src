    ...
    )
{
    ...
    if ((WaitMask & SERIAL_EV_CTS) != 0) {
        ...
        eventsToEnable |= UART_CTS;
    }

    if ((WaitMask & SERIAL_EV_DSR) != 0) {

        eventsToEnable |= UART_DSR;
    }
    ...
    )
{
	...
	DbgPrintEx(1, 0, "-----PL011EvtSerCx2FileOpen!\r\n");
    PL011_DEVICE_EXTENSION* devExtPtr = PL011DeviceGetExtension(WdfDevice);
	...
	{
	    //set the config when uart is open.
    ...
    )
{
	...
	DbgPrintEx(1, 0, "PL011pApplyConfig is enter!\r\n");

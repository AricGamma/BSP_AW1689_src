...
--*/
{
...

    {
        ...
        for(ULONG i = 0; i < resourceCount; i++)
        {
            ...
            {
                pDevice->pRegisters = 
                    (PSUNXII2C_REGISTERS) MmMapIoSpace(
                        res->u.Memory.Start,
                        res->u.Memory.Length,
                        MmNonCached);
...
--*/
{
	...
	AWPRINT(" OnSelfManagedIoCleanup ");
    PPBC_DEVICE pDevice = GetDeviceContext(FxDevice);
...
--*/
{
	...
	AWPRINT(" OnMonitorPowerSettingCallback ");
...
{
    FuncEntry(TRACE_FLAG_SPBDDI);
	AWPRINT(" OnOtherInCallerContext ");
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (descriptor->SerialBusType != I2C_SERIAL_BUS_TYPE)
        {
            ...
            Trace(
                TRACE_LEVEL_ERROR,
                TRACE_FLAG_PBCLOADING,
                "Bus type %c not supported, only I2C - %!STATUS!",
...
{
    FuncEntry(TRACE_FLAG_TRANSFER);
	AWPRINT(" PbcRequestConfigureForNonSequence ");
...
{
    FuncEntry(TRACE_FLAG_TRANSFER);
	AWPRINT(" PbcRequestConfigureForIndex ");
